  ...
  if (
    ...
    try {
      ...
      if (error) {
        ...
        // في وضع البناء، استخدم بيانات فارغة
        if (buildMode) {
          console.warn('⚠️ Build mode: Using empty fallback data')
          return []
        }
      ...
      if (buildMode) {
        console.warn('⚠️ Build mode: Database unavailable, using empty fallback')
        return []
      }
...
export async function getProductById(id, buildMode = false) {
  try {
    ...
    if (error) {
      ...
      if (buildMode) {
        console.warn('⚠️ Build mode: Product not found, using null')
        return null
      }
